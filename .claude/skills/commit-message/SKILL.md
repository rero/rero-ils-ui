...
- Scope: the feature the change touches, as the history already does —
  `circulation`, `acquisition`, `editor`, `document`, `item`, `holdings`,
  `patron`, `menu`, `search`, `translations`. The name of a single project
  under `projects/` serves too when the change is confined to it. Check with
  ...
  scope.
- `!` before the colon marks a breaking change: `feat(circulation)!: ...`.
