  ...
  checkin(barcode: string, transactionLibraryPid: string) {
    ...
    return this.httpClient.post<any>(`${itemApiUrl}/checkin`, {
      ...
      map(data => {
        ...
        } else if (data.action_applied[ItemAction.receive]) {
          item.actionDone = ItemAction.receive;
          loan = data.action_applied[ItemAction.receive];
        } else if (data.action_applied[ItemAction.validate]) {
          item.actionDone = ItemAction.validate;
