...
@Component({
  selector: 'admin-checkbox-is-online',
...
export class CheckboxIsOnlineComponent implements OnInit {
  ...
  validUniqueIsOnlineLocationValidator(control: AbstractControl) {
    if (this.isChecked) {
      ...
      return this.uniqueIsOnlineLocationValidator(
        values.library.$ref,
        values.pid
      );
    } else {
  ...
  uniqueIsOnlineLocationValidator(libraryRef: any, excludePid = null) {
    ...
    let query = `library.pid:${libraryPid} AND is_online:true`;
    if (excludePid) {
      query += ` NOT pid:${excludePid}`;
    }
    return this.recordService.getRecords('locations', query, 0).pipe(
