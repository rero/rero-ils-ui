...
export class MigrationDataDeduplicationBriefComponent implements OnInit {
  ...
  /**
   * Get the badge color from the status.
   *
   * @returns the prime ng severity badge value
   */
  getSeverityBadge(): string {
    let severity = 'secondary';
    const status = this.record()?.metadata?.deduplication?.status;
    if (status === 'match') {
      severity = 'success';
    }
    if (status.endsWith('check')) {
      severity = 'warning';
    }
    return severity;
  }
