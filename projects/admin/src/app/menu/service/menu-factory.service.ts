...
export class MenuFactoryService {
   ...
   *    Variable name must be an attribute of `@rero/shared.User` class prefixed with '$' symbol.
   */
  ...
  private _createMenu(menu: IMenu): MenuItemInterface | undefined {
    ...
    if (menu.name.startsWith('$')) {
      ...
      name = this._userService.user[menu.name.substring(1)];
    } else {
  ...
  private _processRouterLink(menuRef: MenuItemInterface, routerLink: string[]): void {
    const routerL = routerLink.map((link: string) => {
      if (link.startsWith('$')) {
        ...
        link = String(this._userService.user[link.substring(1)]);
      }
      return link;
    });
  ...
  private _processQueryParams(menuRef: MenuItemInterface, queryParams: IMenuKeyValue): void {
    ...
    keys.map((key: string) => {
      if (queryParams[key].startsWith('$')) {
        ...
        queryParams[key] = String(this._userService.user[queryParams[key].substring(1)]);
      }
    });
  ...
  private _hasVariableAvailable(name: string): boolean {
    return MenuFactoryService.AVAILABLE_VARIABLES.includes(name);
  }
}
